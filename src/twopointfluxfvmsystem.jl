...
views vs slices for input of uk, ul
timings on 2D operator

interpolatable data types via fieldnames
...
struct TwoPointFluxFVMSystem
    ...
    function TwoPointFluxFVMSystem(geometry::FVMGraph,problem::FVMPhysics)
        ...
        residual=Array{Float64,2}(undef,geometry.NumberOfNodes,nspec)
        update=Array{Float64,2}(undef,geometry.NumberOfNodes,nspec)
        dirichlet_values=Array{Float64,2}(undef,length(geometry.BPoints),nspec)
...
function unknowns(fvsystem::TwoPointFluxFVMSystem)
    return Array{Float64,2}(undef,fvsystem.geometry.NumberOfNodes,fvsystem.nspec)
end
...
function solve(fvsystem::TwoPointFluxFVMSystem, inival::Array{Float64,2};control=FVMNewtonControl())
    ...
    function eval_and_assemble(fvsystem::TwoPointFluxFVMSystem,U)
        ...
        Y=Array{Float64}(undef,nspec)
        UKL=Array{Float64}(undef,2*nspec)
        # Assemble main part
        for iedge=1:nedges
            K=geom.Edges[iedge,1]
            L=geom.Edges[iedge,2]
