...
with boundary value 1 at r1 and 0 at r2. Test of quadratic convergence.
"""
function maincylindershell(;nref=0,r1=1.0, r2=5.0, z1=0.0, z2=1.0, Plotter=nothing)
    h=0.1*2.0^(-nref)
    R=collect(r1:h:r2)
    Z=collect(z1:h:z2)
    grid=VoronoiFVM.Grid(R,Z)
...
#
function test()
    maindisk() && mainring() && maincylinder() && maincylindershell() && mainsphere() && mainsphereshell()
end
