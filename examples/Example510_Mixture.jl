...
function bcondition(f,u,node,data)
    for species=1:nspec(data)
        boundary_dirichlet!(f,u,node; species, region=data.diribc[1],value=species%2)
        boundary_dirichlet!(f,u,node; species, region=data.diribc[2],value=1-species%2)
    end
end

function main(; n = 10, nspec=5, dim=2, Plotter = nothing, verbose = false, unknown_storage = :dense)
